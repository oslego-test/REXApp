...
EventManager.prototype.bind = function(eventName, func, context){   
    ...
    this.events[eventName].push({fn: func, ctx: context || window})
}
...
function Crawler(settings){
	this._crawlServiceURL = "http://query.yahooapis.com/v1/public/yql?callback=?";   
	// this._crawlServiceURL = "http://localhost/rexapp/offline/?callback=?";  
	this.url = settings.url || ""; 
	this.row_xpath = settings.row_xpath || "";
	this.id = settings.id || "";
	this.bankName = settings.bankName | ""; 
	this.parser = typeof(settings.parser == 'function') ? settings.parser : this.parser; 
...
Crawler.prototype.init = function(){
		...
		$.ajax({
			url: this._crawlServiceURL,
			dataType: "json",
			data: {
				q: 'select * from html where url="' + self.url + '" and xpath=\'' + self.row_xpath + '\'',
				format: "json"
			}
		})
		.success(function(r){
...
Crawler.prototype.parser = function(r){
...
Crawler.prototype.log = function(row){
	this.errorLog.push(row)
}
